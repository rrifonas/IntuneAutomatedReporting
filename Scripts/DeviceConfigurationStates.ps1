...
$outputContainerName = Get-AutomationVariable 'deviceconfigurationstatus' # active container
$snapshotsContainerName = Get-AutomationVariable 'deviceconfigurationstatussnapshots' # historical
...
Function Get-EndpointConfigurationPolicy(){
...
Function Get-DeviceConfigurationPolicyAssignmentStatus(){
...
Function Get-EndpointConfigurationPolicyAssignmentStatus(){
...
$DCP_resource = "deviceManagement/intents"
    ...
    $uri = "https://graph.microsoft.com/$graphApiVersion/$($DCP_resource)/$id/deviceStates"
    (Invoke-RestMethod -Uri $uri -Headers $authToken -Method Get).Value
...
foreach($DCP in $DCPs){
    ...
    $DCPA = Get-DeviceConfigurationPolicyAssignmentStatus -id $id
    write-host "Getting Configuration Policy assignment status..." -f Cyan
...
$ECPs = Get-EndpointConfigurationPolicy

write-host

foreach($ECP in $ECPs){

    write-host "Endpoint Configuration Policy:"$ECP.displayName -f Yellow
    write-host
    #$DCP

    $id = $ECP.id

    $ECPA = Get-EndpointConfigurationPolicyAssignmentStatus -id $id
    write-host "Getting Configuration Policy assignment status..." -f Cyan
    ...
    if($ECPA){
        ...
        if($ECPA.count -gt 1){
            ...
            foreach($device in $ECPA){

                $outputArray += New-Object PSObject -Property @{

                    PolicyName = $ECP.displayName
                    DeviceName = $device.deviceDisplayName
                    UserName = $device.userName
                    Status = $device.state
                }
