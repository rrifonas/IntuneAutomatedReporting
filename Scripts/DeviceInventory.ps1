...
$containerName = Get-AutomationVariable 'deviceInventory' # Resource group name
...
.NOTES
NAME: Get-IntuneDeviceInventory
#>
...
$report = Get-IntuneDeviceInventory -type "Devices"
...
Rename-Item -Path ".\myreport\$csv" -NewName "DeviceInventory.csv"
...
Set-AzStorageBlobContent -Container $ContainerName -File .\myreport\DeviceInventory.csv -Blob DeviceInventory.csv -Force

#Add snapshot file with timestamp
#$date = Get-Date -format "dd-MMM-yyyy_HH:mm"
#$timeStampFileName = "DeviceInventory.csv_" + $date + ".csv"
#Set-AzStorageBlobContent -Container $containerSnapshotName -File '.\myreport\DeviceInventory.csv' -Blob $timeStampFileName -Force 
